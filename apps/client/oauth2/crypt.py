...
try:
    ...
    class PyCryptoVerifier(object):
        ...
        def from_string(key_pem, is_x509_cert):
            ...
            """
            if is_x509_cert:
                raise NotImplementedError('X509 certs are not supported by the PyCrypto library. '
    ...
    class PyCryptoSigner(object):
        ...
        def from_string(key, password='notasecret'):
            ...
            if key.startswith('-----BEGIN '):
                ...
                raise NotImplementedError('PKCS12 format is not supported by the PyCrpto library. '
...
def verify_signature(signed, signature, certs):
    ...
    if not verified:
        ...
        raise AppIdentityError('Invalid token signature. signed: %s signature: %s' % (signed, b64_signature))


def verify_parsed(parsed, signed, signature, certs, audience):
    # Check signature.
    verify_signature(signed, signature, certs)

    # Check creation timestamp.
    iat = parsed.get('iat')
    if iat is None:
        raise AppIdentityError('No iat field in token: %s' % parsed)
    ...
    exp = parsed.get('exp')
    if exp is None:
        raise AppIdentityError('No exp field in token: %s' % parsed)
    if exp >= now + MAX_TOKEN_LIFETIME_SECS:
        raise AppIdentityError('exp field too far in future: %s' % parsed)
    latest = exp + CLOCK_SKEW_SECS

    if now < earliest:
        raise AppIdentityError('Token used too early, %d < %d: %s' % (now, earliest, parsed))
    if now > latest:
        raise AppIdentityError('Token used too late, %d > %d: %s' % (now, latest, parsed))

    # Check audience.
    if audience is not None:
        aud = parsed.get('aud')
        if aud is None:
            raise AppIdentityError('No aud field in token: %s' % parsed)
        if aud != audience:
            raise AppIdentityError('Wrong recipient, %s != %s: %s' % (aud, audience, parsed))
...
def verify_signed_jwt_with_certs(jwt, certs, audience):
    ...
    verify_parsed(parsed, signed, signature, certs, audience)
    return parsed
